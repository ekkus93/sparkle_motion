...
LOG = logging.getLogger("ScriptAgent.entrypoint")
...
def make_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            send_telemetry("tool.ready", {"tool": "ScriptAgent"})
        except Exception:
            pass
        try:
            yield
    ...
    app = FastAPI(title="ScriptAgent Entrypoint (scaffold)", lifespan=lifespan)
    app.state.lock = Lock()
    app.state.ready = False
    app.state.shutting_down = False
    app.state.inflight = 0
    ...
    def invoke(req: RequestModel) -> ResponseModel:
        ...
        try:
            send_telemetry("invoke.received", {"tool": "ScriptAgent", "request_id": request_id})
        except Exception:
            pass
        with app.state.lock:
        ...
        try:
            ...
            def publish_artifact(local_path: str) -> str:
                ...
                try:
                    ...
                    LOG.debug("adk CLI not found; falling back to file URI", extra={"request_id": request_id})
                except Exception:
                    LOG.exception("adk publish attempt failed", extra={"request_id": request_id})
                return f"file://{os.path.abspath(local_path)}"

            artifact_uri = publish_artifact(local_path)
            try:
                send_telemetry("invoke.completed", {"tool": "ScriptAgent", "request_id": request_id, "artifact_uri": artifact_uri})
            except Exception:
                pass
