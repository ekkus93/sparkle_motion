...
- `src/sparkle_motion/production_agent.py` — orchestration agent that sequentially invokes images, TTS, video, QA, assemble stages while persisting `StepExecutionRecord` history.
...
- Each heavy stage lives under `function_tools/` (e.g., `images_sdxl`, `tts_chatterbox`, `videos_wan`, `qa_qwen2vl`, `lipsync_wav2lip`, `assemble_ffmpeg`).
...
- `src/sparkle_motion/run_registry.py` persists run metadata, per-step status, QA summaries, and StageManifest data; `/status` and `/artifacts` serve this state back to UIs.
- Manifests include `plan_id`, `qa_mode`, `qa_skipped`, render profiles, and per-stage metadata so runs can resume reliably.
...
- Stage manifests and QA summaries are persisted under `artifacts/runs/...` and surfaced via notebook helpers (artifacts viewer, final deliverable cell), satisfying the “logging/metrics” visibility expectations.
